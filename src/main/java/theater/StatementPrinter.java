...
public class StatementPrinter {
    ...
    public String statement() {
        ...
        for (Performance performance : getInvoice().getPerformances()) {
            ...
            int thisAmountRenamed = 0;
            thisAmountRenamed = getAmount(performance);

            // add volume credits
            volumeCredits += getVolumeCredits(performance);

            // print line for this order
            result.append(String.format("  %s: %s (%s seats)%n", play.getName(), frmt.format(thisAmountRenamed / Constants
                    .PERCENT_FACTOR), performance.getAudience()));
            totalAmount += thisAmountRenamed;
        }
